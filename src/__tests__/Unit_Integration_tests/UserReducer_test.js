...
describe('User Reducer tests', () => {
  ...
  test('user clear errors', async () => {
    const result = userReducer({
      ...initialUserState,
      error: true,
      errorMessage: 'it doesn\'t work',
    }, { type: 'USER_CLEAR_ERRORS' });
    expect(result).toEqual({ ...initialUserState, errorMessage: undefined, error: false });
  });
  ...
  test('user set has fired action', async () => {
    const alarms = ({
      1:
     { hasFired: false, isActive: true },
    });
    const result = userReducer({
      ...initialUserState,
      alarms,
